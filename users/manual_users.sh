...
if [ ! -f "$SCRIPT_ALIAS_DIR/trebol_user.sh" ]; then
    echo "Creando archivo '$SCRIPT_ALIAS_DIR/trebol_user.sh'..."
    sudo touch "$SCRIPT_ALIAS_DIR/trebol_user.sh"
fi

cat <<EOF | sudo tee "$SCRIPT_ALIAS_DIR/trebol_user.sh" >/dev/null
#!/bin/bash
